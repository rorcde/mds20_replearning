...
def check_model(model, dataset, i, loss, loss_next, loss_prev, previous_sentence, next_sentence, predicted_previous_ids, predicted_next_ids):
    curr_loss = loss.item()
    curr_loss_next = loss_next.item()
    curr_loss_prev = loss_prev.item()
    ...
    print("Iteration {}: last_best_loss = {}, current_loss = {}, current_loss_previous = {}, current_loss_next = {}".format(i, last_best_loss, curr_loss, curr_loss_next, curr_loss_prev))

    print(
        "Previous sentence = {}\n Next_sentence = {}\n Predicted previous sentence = {}\n Predicted next sentence = {}".format(
            text_process_utility.decode_sentences(previous_sentence, dataset.word_dictionary),
            text_process_utility.decode_sentences(next_sentence, dataset.word_dictionary),
            text_process_utility.decode_sentences(predicted_previous_ids, dataset.word_dictionary),
            text_process_utility.decode_sentences(predicted_next_ids, dataset.word_dictionary)
        ))
